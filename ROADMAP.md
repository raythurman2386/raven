...
shipped and verified by `cargo test` (~850 offline tests), clippy `-D warnings`,
fmt.
...
- TUI polish pass (`docs/tui-polish.md`): tool calls as bordered blocks, code-block language labels, Home/End jump-to-top/live, context-sensitive keyhint footer, provider in the top bar, empty-state guidance + error recovery, prompt history recall (Up/Down), wheel/PgUp log scroll (not remapped to history), table-cell width cap, compact `key=value` tool-call args.
