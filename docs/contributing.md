...
- **Token estimate**: implemented by the pure-Rust token estimator in `src/tokenizer.rs` (`count_tokens`), with a ~4% structural overhead bias toward over-estimation so compaction triggers early rather than late.
