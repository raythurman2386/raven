...
- **Token estimate**: implemented by the pure-Rust BPE tokenizer in `src/tokenizer.rs` (`count_tokens`), with a ~4% structural overhead bias toward over-estimation so compaction triggers early rather than late.
...
```bash
cargo test                    # 119 tests (115 unit + 4 CLI smoke), all offline
